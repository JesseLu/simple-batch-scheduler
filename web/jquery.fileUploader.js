	...
	$.fn.fileUploader = function(config){
		...
		config = $.extend({}, {
			...
			callback: function(e) {
                ...
                var step_one_text = '<h3>1. <span id="my_fname"> </span> uploaded.</h3>';
                $("#step_one").replaceWith(step_one_text);
                $("#my_fname").append($(".fname").text());
                $("#step_two").replaceWith('<h2 id="step_two">2. Simulate</h2>');
                $("#s2_test").append($(".fname").text());
				$("#main_container").hide("slow");
                $("#step2_wrapper").show("slow");
                my_status();
                insert_links();
                }
			}
